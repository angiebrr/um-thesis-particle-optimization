...
I developed the \class{Timing System} due to from dissatisfaction with the Visual Studio and Shiny function profilers. This system reliably instruments the multi-threaded engine across all platforms and returns results that are easily parsed. It uses the built-in \class{Performance Timer} from ChilliSource along with its own \func{StartTimer} and \func{StopTimer} static methods in order to time code blocks in a similar fashion to Shiny, as shown in \clis{code:ParticleUpdateTimingSystem}. To achieve this, it uses three hash tables\footnotemark $ $ with the following key-value pairs:
